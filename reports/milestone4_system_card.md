# MSAI605: Milestone 4 System Card

**Authors:** Pramod Goyal, Arun Kulkarni  
**System version:** `milestone4_final`  
**Planned release tag:** `v1.0-final`
...
These values come from `outputs/runs_log.json`, specifically the embedding sweep `run_06` and embedding final test run `run_07`. All release-facing artifacts should cite this same threshold and these same test metrics.
...
For runtime characteristics, see `reports/milestone4_profiling_report.pdf`. That report documents the CPU-only baseline, stage-wise latency breakdown, and batch-size sensitivity. At a high level, embedding inference dominates latency, while preprocessing and cosine-scoring are comparatively small.
...
The helper script `scripts/validate_release_alignment.py` checks that the config, run log, summary, and final report paths are aligned before tagging.
